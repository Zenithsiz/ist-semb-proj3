...
When running the program initially, we'd expect `LOG1` to output for the default level and above. Reading the @logging-docs, we can see that this level depends on the `CONFIG_LOG_DEFAULT_LEVEL` variable, with the following values.
